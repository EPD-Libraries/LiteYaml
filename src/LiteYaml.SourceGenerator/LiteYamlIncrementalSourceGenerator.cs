...
[Generator]
public class LiteYamlIncrementalSourceGenerator : IIncrementalGenerator
{
    private const string YAML_OBJECT_ATTRIBUTE_TYPE_NAME = "LiteYaml.Annotations.YamlObjectAttribute";
        ...
        in SourceProductionContext context)
    {
        try
        {
            ...
            codeWriter.AppendLine("using LiteYaml.Annotations;");
            codeWriter.AppendLine("using LiteYaml.Parser;");
            codeWriter.AppendLine("using LiteYaml.Emitter;");
            codeWriter.AppendLine("using LiteYaml.Serialization;");
            codeWriter.AppendLine();
    ...
    static void EmitCCtor(TypeMeta typeMeta, CodeWriter codeWriter)
    {
        ...
        codeWriter.AppendLine($"__RegisterLiteYamlFormatter();");
    }

    static bool TryEmitRegisterMethod(TypeMeta typeMeta, CodeWriter codeWriter, in SourceProductionContext context)
    {
        codeWriter.AppendLine("[LiteYaml.Annotations.Preserve]");
        using var _ = codeWriter.BeginBlockScope("public static void __RegisterLiteYamlFormatter()");
        codeWriter.AppendLine($"global::LiteYaml.Serialization.GeneratedResolver.Register(new {typeMeta.TypeName}GeneratedFormatter());");
        return true;
    }
