...
{
    [ThreadStatic]
    private static Dictionary<string, int>? _anchorsBufferStatic;

    [ThreadStatic]
    private static ExpandBuffer<ParseState>? _stateStackBufferStatic;
    ...
    Utf8YamlTokenizer _tokenizer;
    ParseState _currentState;
    Scalar? _currentScalar;
    Tag? _currentTag;
    Anchor? _currentAnchor;
    int _lastAnchorId;

    readonly Dictionary<string, int> _anchors;
    readonly ExpandBuffer<ParseState> _stateStack;

    public YamlParser(ReadOnlySequence<byte> sequence)
    {
        _tokenizer = new Utf8YamlTokenizer(sequence);
        _currentState = ParseState.StreamStart;
    ...
    public YamlParser(ref Utf8YamlTokenizer tokenizer)
    {
        ...
        _lastAnchorId = -1;
        _anchors = [];
        _stateStack = new ExpandBuffer<ParseState>(16);
    ...
    public bool Read()
    {
        ...
        if (_currentState == ParseState.End) {
    ...
    void ParseStreamStart()
    {
        ...
        _currentState = ParseState.ImplicitDocumentStart;
        _tokenizer.Read();
    ...
    {
        if (!implicitStarted) {
            while (_tokenizer.CurrentTokenType == TokenType.DocumentEnd) {
                _tokenizer.Read();
            }
        }
        ...
        switch (_tokenizer.CurrentTokenType) {
            ...
            default:
                if (implicitStarted) {
                    ...
                    _currentState = ParseState.BlockNode;
    ...
    void ParseExplicitDocumentStart()
    {
        ...
        PushState(ParseState.DocumentEnd);
        _currentState = ParseState.DocumentContent;
        _tokenizer.Read();
        CurrentEventType = ParseEventType.DocumentStart;
    }
    ...
    void ParseDocumentEnd()
    {
        ...
        // TODO tag handling
        _currentState = ParseState.DocumentStart;
    ...
    void ParseNode(bool block, bool indentlessSequence)
    {
            ...
            case TokenType.Tag: {
                ...
                if (CurrentTokenType == TokenType.Anchor) {
                    ...
                    if (CurrentEventType == ParseEventType.DocumentStart &&
                        _currentTag?.Handle == "!u!") {
                        UnityStrippedMark = _tokenizer.TrySkipUnityStrippedSymbol();
                    }
                    _tokenizer.Read();
                }
                break;
        ...
        switch (CurrentTokenType) {
            case TokenType.BlockEntryStart when indentlessSequence:
                _currentState = ParseState.IndentlessSequenceEntry;
            ...
            case TokenType.DoubleQuotedScaler:
                PopState();
                _currentScalar = _tokenizer.TakeCurrentTokenContent<Scalar>();
                _tokenizer.Read();
                CurrentEventType = ParseEventType.Scalar;
                break;

            case TokenType.FlowSequenceStart:
                _currentState = ParseState.FlowSequenceFirstEntry;
                CurrentEventType = ParseEventType.SequenceStart;
                break;

            case TokenType.FlowMappingStart:
                _currentState = ParseState.FlowMappingFirstKey;
                CurrentEventType = ParseEventType.MappingStart;
                break;

            case TokenType.BlockSequenceStart when block:
                _currentState = ParseState.BlockSequenceFirstEntry;
                CurrentEventType = ParseEventType.SequenceStart;
                break;

            case TokenType.BlockMappingStart when block:
                _currentState = ParseState.BlockMappingFirstKey;
                CurrentEventType = ParseEventType.MappingStart;
                break;

            // ex 7.2, an empty scalar can follow a secondary tag
            case var _ when _currentAnchor != null || _currentTag != null:
                PopState();
                EmptyScalar();
                break;

            // consider empty entry in sequence ("- ") as null
            case TokenType.BlockEntryStart when _currentState == ParseState.IndentlessSequenceEntry:
                PopState();
                EmptyScalar();
                break;

            default: {
                throw new YamlTokenizerException(_tokenizer.CurrentMark,
        ...
        switch (CurrentTokenType) {
            case TokenType.KeyStart:
                ...
                if (CurrentTokenType is
                    ...
                    TokenType.BlockEnd) {
                    _currentState = ParseState.BlockMappingValue;
                    EmptyScalar();
                }
                else {
    ...
    void ParseBlockMappingValue()
    {
        if (CurrentTokenType == TokenType.ValueStart) {
            ...
            if (CurrentTokenType is
                ...
                TokenType.ValueStart or
                TokenType.BlockEnd) {
                _currentState = ParseState.BlockMappingKey;
                EmptyScalar();
            }
            else {
        ...
        switch (CurrentTokenType) {
            ...
            case TokenType.BlockEntryStart:
                ...
                if (CurrentTokenType is TokenType.BlockEntryStart or TokenType.BlockEnd) {
                    _currentState = ParseState.BlockSequenceEntry;
                    EmptyScalar();
                    break;
                }
    ...
    void ParseFlowSequenceEntry(bool first)
    {
        ...
        switch (CurrentTokenType) {
            ...
            case TokenType.KeyStart:
                _currentState = ParseState.FlowSequenceEntryMappingKey;
                _tokenizer.Read();
                CurrentEventType = ParseEventType.MappingStart;
                break;
        ...
        switch (CurrentTokenType) {
            case TokenType.KeyStart:
                ...
                if (CurrentTokenType is
                    ...
                    _currentState = ParseState.FlowMappingValue;
                    EmptyScalar();
                    break;
                }
    ...
    {
        if (empty) {
            _currentState = ParseState.FlowMappingKey;
            EmptyScalar();
            return;
        }
    ...
    void ParseFlowSequenceEntryMappingKey()
    {
        if (CurrentTokenType is
            ...
            _tokenizer.Read();
            _currentState = ParseState.FlowSequenceEntryMappingValue;
            EmptyScalar();
        }
        else {
    ...
    void ParseFlowSequenceEntryMappingValue()
    {
        if (CurrentTokenType == TokenType.ValueStart) {
            ...
            if (CurrentTokenType is
                TokenType.FlowEntryStart or
                TokenType.FlowSequenceEnd) {
                _currentState = ParseState.FlowSequenceEntryMappingEnd;
                EmptyScalar();
            }
            else {
    ...
    {
        _currentState = ParseState.FlowSequenceEntry;
        CurrentEventType = ParseEventType.MappingEnd;
    }
