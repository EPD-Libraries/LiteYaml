...
[Generator]
public class YamlLibraryIncrementalSourceGenerator : IIncrementalGenerator
{
    private const string YAML_OBJECT_ATTRIBUTE_TYPE_NAME = "YamlLibrary.Annotations.YamlObjectAttribute";

    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        IncrementalValuesProvider<(GeneratorAttributeSyntaxContext, Compilation)> provider = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                YAML_OBJECT_ATTRIBUTE_TYPE_NAME,
                static (node, _) => {
        ...
        in SourceProductionContext context)
    {
        try {
            ...
            codeWriter.AppendLine("using YamlLibrary.Annotations;");
            codeWriter.AppendLine("using YamlLibrary.Parser;");
            codeWriter.AppendLine("using YamlLibrary.Emitter;");
            codeWriter.AppendLine("using YamlLibrary.Serialization;");
            codeWriter.AppendLine();
    ...
    static void EmitCCtor(TypeMeta typeMeta, CodeWriter codeWriter)
    {
        ...
        codeWriter.AppendLine($"__RegisterYamlLibraryFormatter();");
    }

    static bool TryEmitRegisterMethod(TypeMeta typeMeta, CodeWriter codeWriter, in SourceProductionContext context)
    {
        codeWriter.AppendLine("[YamlLibrary.Annotations.Preserve]");
        using var _ = codeWriter.BeginBlockScope("public static void __RegisterYamlLibraryFormatter()");
        codeWriter.AppendLine($"global::YamlLibrary.Serialization.GeneratedResolver.Register(new {typeMeta.TypeName}GeneratedFormatter());");
        return true;
    }
